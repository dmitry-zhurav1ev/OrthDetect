...
Зачастую модель никак не определяет этот класс, то есть определяет его как фон. Это приводит к увеличению _false negative_ предсказаний для этого класса и уменьшению метрики recall. Возможно для решения стоит пересмотреть аннотации этого класса и дополнить его новыми данными.
